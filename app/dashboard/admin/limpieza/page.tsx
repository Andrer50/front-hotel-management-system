...
export default function LimpiezaPage() {
  ...
  const { data: commonAreas = [], isLoading: isLoadingAreas } =
    useGetCommonAreasQuery();
  const { data: limpiezas = [], isLoading: isLoadingLimpiezas } =
    useGetLimpiezasQuery();
  ...
  const [selectedLimpieza, setSelectedLimpieza] =
    useState<RegistroLimpieza | null>(null);
  ...
  const filteredItems = useMemo(() => {
    ...
    return combined.filter((item) => {
      ...
      if (activeFilter === "EN PROGRESO")
        return estado === "LIMPIEZA" || (isRoom && room?.estado === "LIMPIEZA");
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {filteredItems.map((item) => {
          if (item.type === "ROOM") {
            ...
            const limpieza = limpiezas.find(
              (l) =>
                l.habitacion === room.id &&
                (l.estado === "EN_PROGRESO" || l.estado === "INSPECCIONADO"),
            );
            const incidencia = incidencias.find(
              (i) => i.habitacion === room.id && i.estado !== "RESUELTO",
            );
            ...
            const incidencia = incidencias.find(
              (i) => i.area_comun === area.id && i.estado !== "RESUELTO",
            );
            return (
              <AreaCleaningCard
                key={item.id}
        ...
        <AlertDialogContent className="rounded-[2rem] border-none p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-dark-secondary font-medium">
              ...
              como disponible. Esta acción notificará a recepción que el espacio
        ...
        <div className="flex flex-col gap-4 mt-auto">
          ...
          <p className="text-[11px] text-dark-secondary italic leading-relaxed line-clamp-2 px-1">
            ...
            {limpieza?.observaciones ||
              "Limpieza profunda de azulejos en el baño y reposición de mini-bar..."}
            &quot;
          </p>
    ...
    if (limpieza?.estado === "INSPECCIONADO") {
      return (
        <div className="p-4 rounded-[1.5rem] bg-[#f1fdf4] border border-[#e2fbe8] flex flex-col gap-3 mt-auto">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-[#00723a] text-white flex items-center justify-center text-[10px] font-bold shadow-sm shadow-[#00723a]/20">
              RM
            </div>
            <div className="flex flex-col">
              <span className="text-xs font-bold text-[#00723a] leading-none">
                Robert Miller
              </span>
              <span className="text-[9px] font-bold text-[#00723a]/60 uppercase tracking-tighter">
                SUPERVISOR CCTV
              </span>
            </div>
          </div>
          <p className="text-[10px] text-[#00723a]/80 font-semibold leading-snug">
            &quot;Control de calidad aprobado. Amenidades VIP preparadas y
            verificadas.&quot;
          </p>
        ...
        <div className="flex flex-col gap-4 mt-auto">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-medium text-dark-secondary tracking-tight">
                Limpieza pospuesta hasta reparación
              </span>
            </div>
          </div>
          <Button
    ...
    return (
      <div className="flex flex-col gap-2 mt-auto">
        <div className="text-[11px] font-bold text-zinc-400">
          Última limpieza:{" "}
          <span className="text-dark-secondary uppercase">12:45 PM</span>
        </div>
  ...
  return (
    <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden bg-white/80 backdrop-blur-sm group hover:shadow-2xl hover:shadow-brand-blue/10 transition-all duration-500 h-[480px] flex flex-col">
      <div className="p-8 flex flex-col h-full">
        ...
        <h3 className="text-5xl font-bold text-dark-primary tracking-tighter mb-6 group-hover:scale-110 transition-transform origin-left">
          {room.numero}
        </h3>
          ...
          <div className="flex flex-col gap-4 mt-auto">
            {incidencia && (
              <div className="flex items-center gap-2 text-red-600/70 font-bold text-[10px] uppercase tracking-tighter">
                ...
                <span>
                  ID: {incidencia.id} - {incidencia.titulo}
                </span>
              </div>
            )}
            <div className="flex items-center gap-2 text-red-600/70 font-bold text-xs uppercase tracking-tighter">
          ...
          <div className="flex flex-col gap-2 mt-auto">
            <div className="text-[11px] font-bold text-zinc-400">
              ...
              <span className="text-dark-secondary uppercase">
                {area.estado_display || "DISPONIBLE"}
              </span>
            </div>
            <button className="text-[11px] font-black text-brand-blue hover:underline uppercase tracking-widest text-left">
              DETALLES DEL ÁREA
