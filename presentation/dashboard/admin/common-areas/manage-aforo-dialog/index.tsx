...
export const ManageAforoDialog = ({
    ...
    const { updateMutation, checkInMutation, checkOutMutation } = useAforoMutations();

    if (!aforo) return null;

    const handleUpdateStatus = (estado: "CONFIRMADA" | "CANCELADA") => {
        updateMutation.mutate({ id: aforo.id, estado }, { onSuccess: () => onOpenChange(false) });
    };
    ...
    const isPending = updateMutation.isPending || checkInMutation.isPending || checkOutMutation.isPending;
                ...
                <div className="p-8 space-y-6">
                    <div className="bg-zinc-50 rounded-2xl p-5 space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Estado</span>
                            <Badge variant="outline" className="font-bold text-[10px] uppercase tracking-widest bg-white">
                                {aforo.estado_display}
                            </Badge>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Huésped</p>
                            <p className="font-medium text-zinc-900">{aforo.huesped_details.nombre_completo}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Área</p>
                            <p className="font-medium text-zinc-900">{aforo.area_comun_nombre}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Ingreso Programado</p>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {(aforo.estado === "COMPLETADA" || aforo.estado === "CANCELADA") && (
