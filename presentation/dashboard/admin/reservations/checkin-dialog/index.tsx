...
import { LogIn, Loader2, BedDouble, User, Calendar } from "lucide-react";
import { useState } from "react";
import { useCheckInMutation } from "@/modules/reservation/domain/hooks/useReservationMutations";
...
export function CheckInDialog({ isOpen, onOpenChange, reservation, onSuccess }: CheckInDialogProps) {
            ...
            { id: reservation.id, observaciones },
            {
                onSuccess: () => {
                    toast.success("Check-in registrado", {
                        description: `${reservation.huesped_nombre} — Hab. ${reservation.habitacion_numero}`,
                    });
                    setObservaciones("");
                    onOpenChange(false);
                    onSuccess();
                ...
                onError: (error) => {
                    toast.error("Error en check-in", {
                        description: error.message || "No se pudo registrar el check-in.",
                    });
                },
            }
        );
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-sm bg-white rounded-2xl p-0 overflow-hidden">
                {/* Header */}
                <div className="bg-gradient-to-br from-emerald-600 to-emerald-500 p-5 text-white">
                    <div className="flex items-center gap-3">
                        <div className="bg-white/20 p-2 rounded-xl">
                            <LogIn className="h-5 w-5" />
                        </div>
                        <div>
                            <DialogTitle className="text-base font-extrabold">Registrar Check-In</DialogTitle>
                ...
                <div className="p-5 flex flex-col gap-4">
                    ...
                    <div className="bg-zinc-50 rounded-xl p-4 flex flex-col gap-2.5">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                            ...
                            <span className="text-dark-secondary">Habitación <strong>{reservation.habitacion_numero}</strong></span>
                        </div>
                        <div className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <Textarea
                            placeholder="Estado del huésped, notas de llegada..."
                            value={observaciones}
                    ...
                    <div className="flex gap-2 mt-1">
                        ...
                        >
                            {checkInMutation.isPending ? (
                                ...
                                <><LogIn className="h-3.5 w-3.5 mr-1.5" /> Confirmar Check-In</>
                            )}
                        </Button>
                    </div>
                </div>
